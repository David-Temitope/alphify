...
export default function KUPurchase({ onSuccess }: KUPurchaseProps) {
  ...
  // Called when user clicks a package or custom buy button
  const handleBuyClick = (units: number, amount: number, label: string, packageType?: string) => {
    ...
    setPendingPurchase({ units, amount, label, packageType });
    setPromoModalOpen(true);
  };

  // Called after promo modal — either with promo or null
  const executePayment = async (promo: PromoInfo | null) => {
    if (!pendingPurchase || !user?.email) return;
    const { units, amount, label, packageType } = pendingPurchase;
    ...
    try {
      ...
      const paystackConfig: any = {
        ...
        callback: (response: PaystackResponse) => {
          void (async () => {
            ...
            const totalUnits = promo ? units + promo.bonusKu : units;
      ...
      // Add Paystack subaccount for split payment if promo has one
      if (promo?.subaccountCode) {
        paystackConfig.subaccount = promo.subaccountCode;
      }
  ...
  const handleCustomPurchase = () => {
    ...
    handleBuyClick(units, units * 3500, `custom_${units}`);
  };
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          {KU_PACKAGES.map((pkg) => {
            ...
            return (
              <button
                key={pkg.id}
                onClick={() => handleBuyClick(pkg.units, pkg.amount, pkg.id, pkg.id)}
      ...
      <PromoCodeModal
        open={promoModalOpen}
        onClose={() => { setPromoModalOpen(false); setPendingPurchase(null); }}
        onProceed={executePayment}
        units={pendingPurchase?.units ?? 0}
